...
class UTTTBoard {
  ...
  constructor(board) {
    if (board) {
      ...
      this.wonBoards = board.wonBoards.slice().map(r => r.slice())
  ...
  performMove(player, index) {
    ...
    if (status !== -1) {
      const newWonBoards = flattenNestedArrays(this.wonBoards)
    ...
    const nextBoardStatus = flattenNestedArrays(this.wonBoards)[localMoveIndex]
  ...
  */
  checkStatus(board) {
    ...
    const isWonBoards = !board
    const n = isWonBoards ? -1 : 0 // value to check against
  ...
  getEmptyPositions() {
    ...
    const emptyPositions = new Array();
    const wonBoardsFlattened = flattenNestedArrays(this.wonBoards)
