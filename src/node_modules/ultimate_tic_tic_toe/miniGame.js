...
class State {
  ...
  getAllPossibleStates() {
    ...
    possibleMoves.forEach(move => {
      const newState = new State({
        board: this.board,
        playerNo: this.getOpponent(),
        visitCount: 0,
        winScore: 0
      })
      newState.board.performMove(this.playerNo, move)
  ...
  randomPlay() {
    ...
    const possibleStates = this.getAllPossibleStates()
    const randomState = possibleStates[Math.floor((Math.random() * possibleStates.length))]
    this.board = randomState.board
    this.playerNo = randomState.playerNo
    this.visitCount = randomState.visitCount
    this.winScore = randomState.winScore
  }
  togglePlayer() {
...
class MonteCarloTreeSearch {
  ...
  findNextMove(board, playerNo) {
    ...
    console.log(winnerNode.state.winScore, winnerNode.state.board.boardValues)
    debugger
...
class UCT {
  ...
  findBestNodeWithUCT(node) {
    ...
    // room for improvement here, also a bug with NaN entries
    // scoreList = scoreList.filter(x => typeof x === 'number' && !isNaN(x))
...
function runGame() {
  ...
  for (let i = 0; i < totalMoves; i++) {
    console.log('board on move', i, board)
  ...
  console.log('win status is a draw',  winStatus, winStatus === board.DRAW, board)
